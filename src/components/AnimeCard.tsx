...
export const AnimeCard: React.FC<AnimeCardProps> = ({ anime, variant = 'standard' }) => {
  ...
  if (variant === 'progress') {
    ...
    return (
      <Link to={`/player/${anime.id}`} className="group relative flex flex-col flex-none w-[320px] bg-akira-card rounded-2xl overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.4)] border border-white/5 hover:-translate-y-1 transition-all">
        <div className="relative aspect-[4/3] w-full overflow-hidden bg-akira-dark">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 bg-black/20">
            <div className="w-12 h-12 rounded-full bg-akira-primary/90 backdrop-blur shadow-lg flex items-center justify-center transform scale-75 group-hover:scale-100 transition-transform duration-300">
        ...
        <div className="flex flex-col p-5 pt-2">
          <h3 className="font-heading font-medium text-white text-[15px] leading-tight line-clamp-1 group-hover:text-akira-primary transition-colors">
  ...
  return (
    <Link to={`/player/${anime.id}`} className="group relative flex flex-col flex-none w-[200px] md:w-[220px]">
      <div className="relative aspect-[3/4] w-full overflow-hidden rounded-2xl bg-akira-card mb-4 shadow-[0_8px_20px_rgba(0,0,0,0.6)] border border-white/5 hover:border-white/10 transition-colors">
        ...
        <div className="absolute top-3 left-3 z-20">
            <div className="bg-akira-primary/95 backdrop-blur-sm text-white text-[10px] font-bold px-2 py-1 rounded-[6px] tracking-wide shadow-md">
              {anime.status === 'Ongoing' ? 'NEW' : 'HOT'}
            </div>
        </div>

        <div className="absolute inset-0 z-20 items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100 hidden md:flex">
        ...
        <button className="absolute top-3 right-3 z-30 w-8 h-8 rounded-full bg-black/40 backdrop-blur border border-white/20 flex items-center justify-center text-white opacity-0 group-hover:opacity-100 hover:bg-akira-primary hover:border-akira-primary transition-all duration-300">
