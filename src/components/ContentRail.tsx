...
export const ContentRail: React.FC<ContentRailProps> = ({ collection, variant = 'new' }) => {
  return (
    <section className="w-full py-8 relative animate-fade-in pl-6 md:pl-10 pr-6 md:pr-10">
      <div className="relative">
        <div className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
             <div className="w-[3px] h-5 bg-akira-primary rounded-full"></div>
             <h2 className="font-sans text-[20px] text-white">
               {collection.title}
             </h2>
          </div>
        ...
        <div className="flex gap-5 overflow-x-auto hide-scrollbar snap-x snap-mandatory pb-4">
          ...
          {variant === 'mood' && collection.animes.map((anime, index) => (
            <div key={`mood-${index}`} className="flex-none w-[160px] h-[190px] bg-akira-card rounded-md overflow-hidden relative group cursor-pointer skew-card shadow-[0_4px_16px_rgba(0,0,0,0.4)] mx-[10px]">
               ...
               <div className="absolute inset-0 bg-gradient-to-t from-akira-dark via-akira-dark/40 to-transparent flex flex-col items-center justify-end pb-5 gap-2">
                  <div className="w-8 h-8 flex items-center justify-center text-akira-primary">
                    <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
                  </div>
                  <span className="font-heading font-bold uppercase text-[11px] tracking-widest text-akira-primary opacity-90">{['EPIC', 'DARK', 'INSPIRING', 'FUN', 'EMOTIONAL', 'MYSTERY'][index % 6]}</span>
               </div>
            </div>
          ))}
          ...
          {variant === 'universe' && [
            ...
            <div key={`stat-${i}`} className="flex-none w-[280px] h-[140px] bg-akira-panel rounded-2xl p-5 flex flex-col justify-between relative overflow-hidden group cursor-pointer border border-white/5 shadow-[0_4px_16px_rgba(0,0,0,0.4)]">
